...
parser.add_argument('--update_policy', type=bool, default=True,
...
parser.add_argument('--perturb_train_samples', action=argparse.BooleanOptionalAction,
    ...
    for j in tqdm(range(args.epochs), desc=f"Learner epochs (iteration {i})"):
        for k in range(num_batches):
            ...
            V_grads, Policy_grads, infos, key, diff = learn.train_step(
                ...
                counterx_indicator = np.concatenate((np.ones(len(batch_C_decrease[k])), 2*np.ones(len(batch_X_decrease[k])))),
