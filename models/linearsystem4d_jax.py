...
class LinearEnv4D(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.init_space = MultiRectangularSet([
            RectangularSet(low=np.array([-0.25, -0.1, -0.25, -0.1]), high=np.array([-0.2, 0.1, -0.2, 0.1]), dtype=np.float32),
            RectangularSet(low=np.array([0.2, -0.1, 0.2, -0.1]), high=np.array([0.25, 0.1, 0.25, 0.1]), dtype=np.float32)
        ])
