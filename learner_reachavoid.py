    ...
    def train_step(self,
        ...
        def loss_fun(certificate_params, policy_params):
            ...
            if len(counterexamples) > 0:
                ...
                losses_init_cx = jnp.maximum(0, V_cx - 1)
                ...
                loss_exp_decrease = (jnp.sum(Vdiffs_trim, axis=0) + jnp.sum(Vdiffs_cx_trim, axis=0)) \
                                        / (jnp.sum(samples_decrease_bool_not_target, axis=0) + jnp.sum(cx_bool_decrease, axis=0) + 1e-6) + 10 * jnp.max(Vdiffs_trim, axis=0)
                ...
                loss_exp_decrease = jnp.sum(Vdiffs_trim, axis=0) / (jnp.sum(samples_decrease_bool_not_target, axis=0) + 1e-6) + 10* jnp.max(Vdiffs_trim, axis=0)
