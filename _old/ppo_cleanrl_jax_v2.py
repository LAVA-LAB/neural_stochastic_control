...
def rollout(
    ...
    for step in range(0, args.num_steps):
        ...
        noise = jnp.zeros(2)
        next_obs, next_reward, terminated, truncated, infos = step_method(next_obs, jax.device_get(action), noise)
...
def train_step(
        ...
        action_key: jax.Array,
        storage: Storage
) -> tuple[jax.Array, jax.Array, jax.Array, jax.Array, Storage]:
...
def loop_body(i, val):
    step = i
    next_obs, action_key, storage = val
    ...
    # TRY NOT TO MODIFY: execute the game and log data.
    noise = jnp.zeros(2)
    next_obs, next_reward, terminated, truncated, infos = env.step_flax(next_obs, jax.device_get(action), noise)
...
def train_num_steps(
    ...
    val = (next_obs, action_key, storage)
    next_obs, next_done, action_key, storage = jax.lax.fori_loop(0, args.num_steps, loop_body, val)

    # for step in range(0, args.num_steps):
    #     next_obs, next_done, action_key, storage = train_step(step, step_method, agent_state, next_obs, action_key, storage)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        ticDiff()
        next_obs, next_done, action_key, storage = train_num_steps(env.step_flax, agent_state,
                                                          next_obs, next_done, action_key, storage)
        tocDiff()
